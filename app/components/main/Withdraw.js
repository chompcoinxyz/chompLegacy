      ...
      <div className="w-full flex flex-col justify-between">
        <form onSubmit={handleSubmit(onWithdraw)} className="w-full mt-6">
          ...
          <button 
            type="submit" 
            onClick={onWithdraw}
            className={`w-full h-[60px] text-[17px] flex flex-row items-center justify-center font-bold px-6 py-2 text-white  mt-[13px] rounded-[10px] uppercase ${!withdrawLoading && amountWithdraw > 0 ? 'bg-primary hover:bg-blue-700': 'bg-btnDisabled text-btnGray'}`}
            disabled={!withdrawLoading && amountWithdraw > 0 ? false : true}
          >
