...
export default function NftCard({ nft, mintLoading, onMint }) {
  ...
  return (
    <div key={nft.tokenId} className="w-full flex bg-black shadow rounded-[19px] p-[20px] flex-col items-center">
      ...
      <button 
        ...
        className={`w-full h-[45px] text-[17px] flex flex-row items-center justify-center font-bold px-6 text-white rounded-[10px] uppercase ${!mintLoading || nft.maxIssuance === nft.totalSupply ? 'bg-gradient-to-r from-[#F88040] to-[#FD603D] hover:opacity-80' : 'bg-btnDisabled text-white'}`}
      >
